...
server.send(json.dumps({'process_id': 2, 'type': 'CON'}))
time.sleep(5)
server.send(json.dumps({'process_id': 2 , 'clock': 1 , 'type': 'REQ','req_number': 1}))
time.sleep(5)
server.send(json.dumps({'req_process_id': 1 , 'reply_process_id': 2, 'clock': 3, 'type': 'REP','req_number': 1}))
